...
class DisplayBuffer:
        ...
        with self._lock:
            ...
            if text.strip():
                self.segments.append(
                    TranscriptSegment(
                        text=text.strip(), is_final=False, confidence=confidence
                    )
                )
    ...
    def add_final(self, text: str) -> None:
        ...
        with self._lock:
            ...
            if text.strip():
                self.segments.append(
                    TranscriptSegment(text=text.strip(), is_final=True, confidence=1.0)
                )
    ...
    def _notify_update(self) -> None:
        ...
        if self._on_update:
            ...
            is_final = (
                all(s.is_final for s in self.segments) if self.segments else False
            )
