...
# not from a sibling version (e.g., a parallel stable checkout)
...
def main():
    try:
        ...
        # Same AppUserModelID as the main app so the splash's taskbar
        # presence groups under Aria with the Aria icon (must be set
        # before QApplication creates a window).
        ...
        # Set app-wide window icon so the taskbar/Alt+Tab does not fall
        # back to a default Windows icon.
        try:
            _icon_path = os.path.join(_project_dir, "assets", "aria.ico")
