...
class ReminderDialog(QWidget):
    ...
    ACCENT_ORANGE = QColor(255, 140, 0)  # #FF8C00
    ...
    def _init_ui(self):
        ...
        self._dismiss_btn.setStyleSheet(
            ...
            QPushButton:hover {{ background: rgba(255, 140, 0, 255); }}
            """
        )
    ...
    def show_confirm(
        ...
        self._title.setStyleSheet(
            f"color: {self.ACCENT_ORANGE.name()}; font-size: 14px; "
            f"font-weight: bold; background: transparent;"
        )
    ...
    def show_notify(
        ...
        elif repeat_interval_seconds:
            self._icon.setText("\U0001F501")
            self._title.setText("重复提醒")
        else:
    ...
    def dismiss_cancelled(
        self, reminder_ids=(), dismiss_active: bool = False
    ) -> bool:
        """Dismiss the visible popup when its reminder was cancelled."""
        if not self.isVisible():
            return False
        ids = {str(reminder_id) for reminder_id in reminder_ids if reminder_id}
        if not dismiss_active and self._reminder_id not in ids:
            return False
        _dlog(
            f"Voice cancel dismiss: id={self._reminder_id}, "
            f"dismiss_active={dismiss_active}"
        )
