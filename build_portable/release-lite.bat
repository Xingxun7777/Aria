...
call :get_version
set "ARIA_DIST_NAME=Aria_release_lite"
set "ARIA_ARCHIVE_NAME=Aria-v%ARIA_VERSION%-lite.7z"

echo [INFO] 用途: GitHub 发布 / 首次运行自动下载 ASR 模型
...
echo [1/3] 打包 Lite 便携版...
.venv\Scripts\python.exe build_portable\build.py --dist-name %ARIA_DIST_NAME%
if errorlevel 1 (
    echo 打包失败！
    pause
    exit /b 1
...
echo   3. 发布位置: GitHub Releases（lite）
echo.
pause
exit /b 0

:dry_run
    echo [DRY-RUN] .venv\Scripts\python.exe build_portable\build.py --dist-name %ARIA_DIST_NAME%
