...
def _compile_stage(stage_dir: Path) -> bool:
    ...
    # workers MUST stay 1 (sequential, in-process). workers=0 spawns a
    # ProcessPoolExecutor whose Windows workers re-import the parent's
    # __main__ (launcher.py); its module-level singleton lock then exits the
    # worker → BrokenProcessPool → every in-app update fails. Verified by
    # e2e repro on the 1.0.3.19 portable bundle. ~600KB of sources compile
    # sequentially in <2s, so parallelism buys nothing here anyway.
    ok = compileall.compile_dir(str(stage_dir), quiet=2, workers=1)
    if not ok:
        return False
...
def download_and_stage(manifest: dict) -> dict:
    ...
    try:
        ...
        # so apply_staged_update works even for upgrades from older versions
        # that didn't ship these files via the previous update.bat path.
