...
_CN_NUMBER_TOKEN = r"[零〇一壹二贰两三叁四肆五伍六陆七柒八捌九玖十拾\d]"


def _normalize_relative_markers(text: str) -> str:
    """Normalize colloquial relative-time markers into the existing grammar.

    Chinese speakers commonly say ``过一个小时提醒我`` without the trailing
    ``后``.  A leading ``过`` is itself an unambiguous relative-time marker, so
    convert only complete duration phrases and leave unrelated words such as
    ``过生日`` untouched.
    """
    text = text.replace("之后", "后").replace("以后", "后")
    text = text.replace("半个小时", "半小时")

    patterns = (
        # Composite duration must run first so it is not cut after the hour.
        rf"过(({_CN_NUMBER_TOKEN}+个?小时{_CN_NUMBER_TOKEN}+分(?:钟)?))(?!后)",
        r"过(半小时)(?!后)",
        rf"过(({_CN_NUMBER_TOKEN}+个?(?:小时|天|周|星期|礼拜|月)))(?!后)",
        rf"过(({_CN_NUMBER_TOKEN}+分钟))(?!后)",
    )
    for pattern in patterns:
        text = re.sub(pattern, r"\1后", text)
    return text
...
def parse_chinese_time(text: str, now: datetime = None) -> Optional[datetime]:
    ...
    # Normalize synonyms and colloquial relative expressions.
    text = _normalize_relative_markers(text.strip())
...
def parse_reminder_text(
    ...
    text = re.sub(r"[，,。.！!？?；;：:、]", "", text)
    text = _normalize_relative_markers(text)
...
# ============================================================================
# Full reminder request parsing (one-shot + fixed-interval recurrence)
# ============================================================================

_REPEAT_DURATION_RE = re.compile(
    rf"每(?:隔)?(?P<duration>"
    rf"半(?:个)?小时|"
    rf"{_CN_NUMBER_TOKEN}+个?小时|"
    rf"{_CN_NUMBER_TOKEN}+分钟|"
    rf"{_CN_NUMBER_TOKEN}+天|"
    rf"小时|分钟|天"
    rf")"
)
...
def _duration_seconds(duration: str) -> int:
    """Convert a supported fixed duration to seconds, or return 0."""
    duration = duration.strip().replace("半个小时", "半小时")
    if duration == "半小时":
        return 30 * 60
    if duration == "小时":
        return 60 * 60
    ...
    normalized = _normalize_chinese_nums(duration)
    match = re.fullmatch(r"(\d+)个?小时", normalized)
    if match:
        return int(match.group(1)) * 60 * 60
    match = re.fullmatch(r"(\d+)分钟", normalized)
    if match:
        return int(match.group(1)) * 60
    match = re.fullmatch(r"(\d+)天", normalized)
    if match:
...
def parse_reminder_request(
    text: str, now: datetime = None
) -> Tuple[Optional[str], Optional[datetime], int]:
    """Parse a one-shot or fixed-interval reminder request.

    Returns ``(content, first_trigger_time, repeat_interval_seconds)``.
    A zero repeat interval preserves the legacy one-shot contract.

    Supported recurring examples include ``每三十分钟提醒我喝水``、
    ``每隔半个小时提醒我站起来`` and ``每两小时提醒我一次休息``.
    Complex calendar recurrence (weekdays/monthly rules) is intentionally not
    inferred here.
    """
    if not text or not text.strip():
        return None, None, 0
    ...
    repeat_match = _REPEAT_DURATION_RE.search(clean)
    if not repeat_match:
        content, trigger_time = parse_reminder_text(text, now)
        return content, trigger_time, 0

    duration = repeat_match.group("duration")
    interval_seconds = _duration_seconds(duration)
    if interval_seconds < 60:
        return None, None, 0

    remaining = clean[: repeat_match.start()] + clean[repeat_match.end() :]
    # In a recurring sentence, “提醒我一次” means one notification per cycle,
    # not that the content itself begins with “一次”.
    remaining = re.sub(
        r"(帮我提醒|提醒我|提醒一下|定时提醒)(?:一次)",
        r"\1",
        remaining,
        count=1,
    )
    first_duration = {"小时": "一小时", "分钟": "一分钟", "天": "一天"}.get(
        duration, duration
    )
    first_request = f"{first_duration}后{remaining}"
    content, trigger_time = parse_reminder_text(first_request, now)
    if trigger_time is None:
        return None, None, 0
    return content, trigger_time, interval_seconds


def format_repeat_interval(interval_seconds: int) -> str:
    """Human-readable fixed recurrence interval."""
    seconds = max(0, int(interval_seconds or 0))
