...
理由：编程语境 + cloud code是明显的误识别

【不要替换的情况 - 非常重要】
原文：现在这个头发点零零四又有问题。
修正：现在这个头发0.004又有问题。
理由："点零零四"是数字0.004的口语说法，应转为数字格式，不是任何工具名的乱码

原文：我刚才试了一下风川祥子。
修正：我刚才试了一下风川祥子。
理由："风川祥子"是人名/角色名，表意完整通顺，绝对不能替换成其他词
...
class AIPolisher:
    ...
    def _build_prompt(self, text: str, screen_context: str = "") -> str:
        ...
        if self.config.auto_structure:
            feature_parts.append(
                "- 当内容包含递进关系（首先/其次/然后/最后）、并列要点、"
                "或步骤说明时，用编号列举（1. 2. 3.），每个要点独占一行。"
                "禁止使用Markdown格式符（不要用#、*、-），只输出纯文本+编号+换行"
            )
