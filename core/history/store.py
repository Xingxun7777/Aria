...
class HistoryStore:
    ...
    Usage:
        ...
        records = store.query(date="YYYY-MM-DD")
    """
    ...
    def recent(
        ...
        """
        Get the newest records across day files, newest first.

        Scans day files newest-first and stops as soon as `limit` records
        are collected, so the common case reads a single file.

        Args:
            record_type: Filter by record type (None = all types)
            limit: Maximum records to return
            max_days: How many newest day files to consider

        Returns:
            List of HistoryRecord, newest first
        """
        if not self.enabled or limit <= 0:
            return []

        records: List[HistoryRecord] = []
        for date_str in self.get_dates(max_days=max_days):
            remaining = limit - len(records)
            if remaining <= 0:
                break
            records.extend(
                self.query(date=date_str, record_type=record_type, limit=remaining)
            )
        return records[:limit]

    def latest_asr_text(self, max_records: int = 50) -> str:
        """Return the newest recoverable ASR text across day files.

        Prefer the final/polished output and fall back to the raw transcript.
        Historical wakeword/command markers were stored as ``ASR`` records by
        older releases; they are operational events, not transcript recovery
        candidates, so skip them here.
        """
        if max_records <= 0:
            return ""
        for record in self.recent(
            ...
            if record.metadata.get("recoverable") is False:
                continue
            # Older cloud-rescue records did not distinguish accepted late
            # transcripts from hallucination/length rejects. Fail closed for
            # those legacy rows; new accepted rescues persist recoverable=True.
            if (
                record.metadata.get("rescued") is True
                and record.metadata.get("recoverable") is not True
            ):
                continue
            text = (record.output_text or record.input_text or "").strip()
            if not text:
                continue
            if text.startswith(("[命令]", "[唤醒词]")):
                continue
            return text
        return ""
    ...
    def get_dates(self, max_days: int = 30) -> List[str]:
        ...
        for f in sorted(self.data_dir.glob("*.jsonl"), reverse=True):
            date_str = f.stem  # e.g., "YYYY-MM-DD"
