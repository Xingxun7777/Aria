...
class AudioCapture:
    ...
    def __init__(self, config: Optional[AudioConfig] = None):
        ...
        # Manual-stop VAD telemetry: snapshot taken by stop() before the VAD
        # buffers are drained; consumed once by the caller (app hotkey-stop
        # path) so the segment JSON gets endpoint stats even without an EOS.
        self._last_stop_vad_stats = None
    ...
    def set_callbacks(
        ...
        if self._vad:
            self._vad.set_callbacks(
                ...
                on_speech_soft_split=on_speech_soft_split,
            )
    ...
    def _audio_callback(self, indata, frames, time_info, status):
        ...
        if self._vad:
            ...
            elif event == "speech_soft_split" and speech_audio is not None:
                # Mid-utterance natural-pause split: segment flushed to ASR,
                # but user is still speaking — keep streaming state intact.
                self._speech_samples += len(speech_audio)
                try:
                    self._speech_queue.put_nowait(("speech_soft_split", speech_audio))
                except queue.Full:
                    logger.warning("Speech queue full, dropping soft-split segment")
    ...
    def stop(self) -> Optional[np.ndarray]:
        ...
        with self._lock:
            ...
            # Snapshot in-flight VAD segment telemetry BEFORE draining buffers
            # below (observation only). Safe: the stream is closed, so no
            # audio callback can mutate the accumulators concurrently.
            self._last_stop_vad_stats = None
            if self._vad:
                try:
                    self._last_stop_vad_stats = self._vad.take_manual_stop_stats()
                except Exception as e:
                    logger.debug(f"Manual-stop VAD stats failed: {e}")
    ...
    def consume_stop_vad_stats(self):
        """Return and clear the VAD stats snapshot from the last stop().

        None when nothing was in flight at stop time (e.g. EOS already fired
        and committed the segment through the normal callback path).
        """
        stats = self._last_stop_vad_stats
        self._last_stop_vad_stats = None
        return stats
