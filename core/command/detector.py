...
class CommandDetector:
    ...
    def _load_config(self, config_path: Optional[str] = None) -> None:
        ...
        if not config_path.exists():
            # commands.json is a user runtime file (untracked, like
            # wakeword.json); fresh checkouts only have the template, so
            # fall back to it for the stock command set.
            template_path = config_path.with_name("commands.template.json")
            if template_path.exists():
                logger.info(
                    f"Command config missing, using template: {template_path}"
                )
