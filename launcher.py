...
# === Windows taskbar identity (must run before any Qt window is created) ===
# Aria launches via a renamed pythonw copy (Aria.exe) from a .bat. Without an
# explicit AppUserModelID, Windows derives a heuristic taskbar identity from the
# process and can show a wrong / stale taskbar icon for Aria's windows (notably
# the Settings window). Pinning an explicit, stable AppID makes the taskbar,
# Alt-Tab and jump list always identify as Aria and use our own window icon.
...
def _migrate_user_payload_after_swap(install_root, state):
    """Carry user config/data/models from aria.backup.TS into the fresh live tree.

    Logic lives in core/update_migration.py (stdlib-only, unit-tested). The
    import is lazy + guarded: this runs from the NEW live tree's launcher, so
    core/ is the zip-verified fresh code, but a failed import must never block
    boot — losing the carry-over is strictly better than not starting.
    """
    try:
        from core.update_migration import migrate_user_payload_after_swap
    except ImportError:
        try:
            from aria.core.update_migration import migrate_user_payload_after_swap
        except ImportError:
            return
    try:
        migrate_user_payload_after_swap(install_root, state)
    except Exception as e:
        print(f"[UPDATE] payload migration error: {e}")


def _reclaim_payload_after_rollback(live, retired):
    """After backup→live restore, payload dirs migrated into the failed live
    (now retired) must move back, or the restored old version would boot
    without data/ and models/. The restored OLD tree may predate
    core/update_migration.py, so the logic is inlined here (launcher.py itself
    is the NEW file performing the rollback)."""
...
def _do_rollback(install_root, state):
    ...
    if os.path.isdir(retired):
        _reclaim_payload_after_rollback(live, retired)
...
def _handle_update_state_at_boot():
    ...
    if status == "swapped":
        # Carry user config/data/models out of the backup dir BEFORE anything
        # reads config. Must run here (new live tree's code) because the swap
        # itself may have been performed by an older updater_runner.
        _migrate_user_payload_after_swap(install_root, state)
...
# === Singleton Check ===
# Two layers:
#   OUTER — cross-install global mutex (core/singleton.py): one Aria per
#           machine regardless of install directory. Field forensics
#           (2026-07-19 slim trial) showed dev + slim running together both
#           capture the mic and double-insert every sentence, while the
#           second instance silently loses the hotkey registration race.
#   INNER — per-install named mutex + temp lock file (below): unchanged
#           legacy protection with stale-lock cleanup and PID validation.
# The import is guarded like core.update_migration: a broken helper must
# never block boot (fail-open keeps the inner layer active).
try:
    from core.singleton import (
        allow_parallel_personal_test_instance,
        notify_already_running_and_exit,
        release_global_mutex,
        try_acquire_global_mutex,
    )
except ImportError:
    try:
        from aria.core.singleton import (
            allow_parallel_personal_test_instance,
            notify_already_running_and_exit,
            release_global_mutex,
            try_acquire_global_mutex,
        ...
        def notify_already_running_and_exit(message: str = "") -> None:
            sys.exit(1)


# Per-install named mutex + lock file (INNER layer).
# Dev and portable keep different names so the inner layer never blocks
# across trees — cross-tree exclusion is the OUTER global mutex's job.
if IS_PORTABLE:
    ...
    MUTEX_NAME = "Aria-Singleton-Mutex"
...
def acquire_lock():
    ...
    # OUTER layer: cross-install global mutex (dev tree vs portable installs).
    if not try_acquire_global_mutex():
        return False

    # INNER layer: per-install Windows Named Mutex (more reliable)
...
def acquire_lock_with_retry(timeout: float = 6.0) -> bool:
    """Acquire singleton lock, briefly waiting for a just-quit instance.

    Windows destroys the named mutex only when the previous process fully exits.
    Aria's tray icon disappears before backend cleanup is done, so an immediate
    double-click used to hit the still-held mutex and the second launcher would
    silently exit.  Retrying preserves strict single-instance behavior while
    letting the user's second click become the next launch as soon as cleanup
    finishes.
    """
    if acquire_lock():
        return True
    ...
    print(f"[LOCK] Existing Aria instance detected; waiting up to {timeout:.1f}s")
    deadline = time.monotonic() + timeout
    delay = 0.12
    while time.monotonic() < deadline:
        time.sleep(delay)
        if acquire_lock():
            print("[LOCK] Previous instance exited; continuing startup")
            return True
...
def release_lock():
    ...
    # Release mutex (inner per-install, then outer cross-install)
    release_mutex()
    release_global_mutex()
...
# Check singleton before doing anything expensive.  The retry window fixes the
# common tray-quit → immediate double-click case without allowing two live
# instances to overlap.
_parallel_personal_test = allow_parallel_personal_test_instance(
    _get_install_root()
)
_singleton_retry_timeout = os.environ.get("ARIA_SINGLETON_RETRY_TIMEOUT", "6.0")
if _parallel_personal_test:
    print(
        "[LOCK] PERSONAL TEST ONLY: parallel-instance singleton bypass is active"
    )
elif not acquire_lock_with_retry(_singleton_retry_timeout):
    print(f"Lock file: {LOCK_FILE}")
    print("If Aria is not visible, check system tray.")
    print("If stuck, run: python tools/kill_and_restart.py")
    # Covers both conflicts: same install dir (inner mutex/lock) and another
    # install dir (outer global mutex). Shows a native dialog — under
    # pythonw.exe stdout is devnull, prints alone are invisible — then exits 1.
    notify_already_running_and_exit()
...
try:
        ...
        if asr_engine == "funasr":
            ...
            if model_name == "auto":
                try:
                    ...
                    log(f"Auto-selected Qwen3 model: {model_reason} -> {model_name}")
                except Exception:
            ...
            qwen3_pre_cfg = dict(qwen3_cfg)
            qwen3_pre_cfg["model_name"] = model_name
            pre_config = Qwen3Config.from_mapping(qwen3_pre_cfg)
    ...
    from aria.progress_ipc import StartupReadyNotifier

    log("Import OK, starting...")

    emit_progress("audio_capture", "正在创建悬浮窗...", 95)

    def _finish_splash_after_ui_ready():
        log("Floating ball visible; signaling splash done")
        emit_progress("done")
        if reporter:
            reporter.close()

    ui_ready_notifier = StartupReadyNotifier(_finish_splash_after_ui_ready)

    # Run main application
    exit_code = main(on_ui_ready=ui_ready_notifier.notify)
    ...
    log(traceback.format_exc())
    try:
        _reporter = globals().get("reporter")
        _notifier = globals().get("ui_ready_notifier")
        if _reporter and not (_notifier and _notifier.notified):
            _reporter.error("启动失败，请查看启动日志")
            _reporter.close()
    except Exception:
        pass
    sys.exit(1)
